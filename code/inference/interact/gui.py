...
class App(QWidget):
    def __init__(self, net: XMem, 
                ...
                fbrs_ctrl: Optional[FBRSController], config, device):
        super().__init__()
        ...
        # set window
        self.setWindowTitle('IM-XMem Demo')
        ...
        self.radio_s2m = QRadioButton('Prompt')
        ...
        if fbrs_ctrl is not None:
            self.curr_interaction = 'Click'
            self.radio_fbrs.toggle()
        else:
            self.curr_interaction = 'Scribble'
            self.radio_s2m.setChecked(True)
        self.radio_fbrs.setVisible(False)
    ...
    def on_mouse_press(self, event):
        ...
        if self.curr_interaction == 'Scribble':
            ...
            if self.fbrs_controller is None:
                return
