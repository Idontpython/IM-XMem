...
class XMem(nn.Module):
    ...
    def load_weights(self, src_dict, init_as_zero_if_needed=False):
        # Allow loading weights from a previously saved model

        model_dict = self.state_dict()
        
        # 1. Create a new state dict to fill with weights from src_dict
        new_state_dict = {}
        
        # 2. Iterate through the source dictionary and copy weights
        for k, v in src_dict.items():
            if k in model_dict:
                if v.shape == model_dict[k].shape:
                    new_state_dict[k] = v
                else:
                    # Handle the size mismatch for the transform layer (our innovation)
                    print(f"Partially loading {k} due to shape mismatch: src {v.shape}, dst {model_dict[k].shape}")
                    if k == 'decoder.hidden_update.transform.weight':
                        # This is our special case for the GRU input transformation
                        dst_v = model_dict[k]
                        # Copy the part of the weights that corresponds to the original inputs
                        dst_v[:, :v.shape[1], :, :] = v
                        new_state_dict[k] = dst_v
        
        # 3. Load the new state dict, ignoring missing keys (like our new detail extractors)
