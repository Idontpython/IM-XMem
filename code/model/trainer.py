...
class XMemTrainer:
    def __init__(self, config, logger=None, save_path=None, local_rank=0, world_size=1):
        ...
        # Freeze parameters if requested (train only detail branches and transform)
        if int(config.get('train_detail_only', 0)) == 1:
            for name, p in self.XMem.module.named_parameters():
                p.requires_grad = False
            # Unfreeze detail extractors and HiddenUpdater.transform
            def unfreeze(n):
                for name, p in self.XMem.module.named_parameters():
                    if name.startswith(n):
                        p.requires_grad = True
            unfreeze('decoder.g4_detail_extractor')
            unfreeze('decoder.g8_detail_extractor')
            unfreeze('decoder.hidden_update.transform')
            print('train_detail_only enabled: only detail extractors and HiddenUpdater.transform are trainable.')

        # Freeze all parameters except BGMA modules
        if int(config.get('train_bgma_only', 0)) == 1:
            # First freeze everything
            for name, p in self.XMem.module.named_parameters():
                p.requires_grad = False
            
            # Then unfreeze only BGMA modules
            bgma_param_count = 0
            for name, p in self.XMem.module.named_parameters():
                if 'bgma' in name:
                    p.requires_grad = True
                    bgma_param_count += p.numel()
            
            print(f'train_bgma_only enabled: only BGMA modules are trainable.')
            print(f'Trainable BGMA parameters: {bgma_param_count:,}')

        # Freeze all parameters except sensory memory branch (decoder.hidden_update.*)
        if int(config.get('train_sensory_only', 0)) == 1:
            # Freeze all first
            for name, p in self.XMem.module.named_parameters():
                p.requires_grad = False
            # Unfreeze only sensory memory branch
            sensory_param_count = 0
            for name, p in self.XMem.module.named_parameters():
                if name.startswith('decoder.hidden_update'):
                    p.requires_grad = True
                    sensory_param_count += p.numel()
            print('train_sensory_only enabled: only decoder.hidden_update.* are trainable.')
            print(f'Trainable sensory parameters: {sensory_param_count:,}')

        # Freeze all parameters except MSF modules
        if int(config.get('train_msf_only', 0)) == 1:
            # Freeze all first
            for name, p in self.XMem.module.named_parameters():
                p.requires_grad = False
            # Unfreeze only MSF modules and temporal residual scalars
            msf_param_count = 0
            for name, p in self.XMem.module.named_parameters():
                if ('msf' in name) or ('delta_alpha' in name):
                    p.requires_grad = True
                    msf_param_count += p.numel()
            print('train_msf_only enabled: only MSF modules and temporal residual params are trainable.')
            print(f'Trainable MSF parameters: {msf_param_count:,}')

        # Freeze all parameters except MSF + MU-GRU motion modules
        if int(config.get('train_msf_mugru_only', 0)) == 1:
            # Freeze all first
            for name, p in self.XMem.module.named_parameters():
                p.requires_grad = False
            # Unfreeze only MSF modules and MU-GRU motion_gate_conv
            msf_mugru_param_count = 0
            for name, p in self.XMem.module.named_parameters():
                if ('msf' in name) or ('motion_gate_conv' in name):
                    p.requires_grad = True
                    msf_mugru_param_count += p.numel()
            print('train_msf_mugru_only enabled: only MSF modules and MU-GRU motion_gate_conv are trainable.')
            print(f'Trainable MSF + MU-GRU parameters: {msf_mugru_param_count:,}')

        # Freeze all parameters except MSF + MU-GRU + BGMA modules (all improvements)
        if int(config.get('train_improvements_only', 0)) == 1:
            # Freeze all first
            for name, p in self.XMem.module.named_parameters():
                p.requires_grad = False
            # Unfreeze MSF modules, MU-GRU motion_gate_conv, and BGMA modules
            improvements_param_count = 0
            for name, p in self.XMem.module.named_parameters():
                if ('msf' in name) or ('motion_gate_conv' in name) or ('bgma' in name):
                    p.requires_grad = True
                    improvements_param_count += p.numel()
            print('train_improvements_only enabled: only MSF + MU-GRU + BGMA modules are trainable.')
            print(f'Trainable improvement parameters: {improvements_param_count:,}')

        # 移除本次实验的只训 f4/f16 冻结逻辑，恢复为默认训练配置
    ...
    def do_pass(self, data, it=0):
        ...
        # Reset decoder hidden updater temporal buffer for each new sequence/batch
        decoder = getattr(self.XMem.module, 'decoder', None)
        if decoder is not None and hasattr(decoder, 'hidden_update') \
                and hasattr(decoder.hidden_update, 'reset_temporal_buffer'):
            decoder.hidden_update.reset_temporal_buffer()
    ...
    def save_checkpoint(self, it):
        ...
        # Also save pure network weights (state_dict) for evaluation convenience
        model_path = f'{self.save_path}_{it}.pth'
        torch.save(self.XMem.module.state_dict(), model_path)
        print(f'Network weights (state_dict) saved to {model_path}.')
    ...
    def load_checkpoint(self, path):
        ...
        # Robust partial load to support architectural changes
        model_dict = self.XMem.module.state_dict()
        new_state_dict = {}
        for k, v in network.items():
            if k in model_dict:
                if v.shape == model_dict[k].shape:
                    ...
                    if k == 'decoder.hidden_update.transform.weight':
                        dst = model_dict[k]
                        dst[:, :v.shape[1], :, :] = v
                        new_state_dict[k] = dst

        # Load with strict=False so new params (detail extractors) stay randomly initialized
        self.XMem.module.load_state_dict(new_state_dict, strict=False)

        # Optimizer/scheduler may mismatch; load best-effort
        try:
            self.optimizer.load_state_dict(optimizer)
            self.scheduler.load_state_dict(scheduler)
            print('Optimizer and scheduler states loaded.')
        except Exception as e:
            print(f'Optimizer/Scheduler state not loaded due to mismatch: {e}')
