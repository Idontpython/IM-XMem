...
class Configuration():
    def parse(self, unknown_arg_ok=False):
        ...
        # for freezing some parts of the model (keep legacy deep_update_all; remove experiment-only flag)
        parser.add_argument('--deep_update_all', action='store_true')
        ...
        # Accept both --local_rank (legacy) and --local-rank (torchrun/launch)
        parser.add_argument('--local_rank', '--local-rank', default=0, type=int, help='Local rank of this process')

        # Fine-tune control
        parser.add_argument('--train_detail_only', default=0, type=int,
                            help='If 1, train only detail branches and HiddenUpdater.transform; freeze others')
        parser.add_argument('--train_bgma_only', default=0, type=int,
                            help='If 1, train only BGMA modules; freeze all other parameters')
        parser.add_argument('--train_sensory_only', default=0, type=int,
                            help='If 1, train only sensory memory branch (decoder.hidden_update.*); freeze others')
        parser.add_argument('--train_msf_only', default=0, type=int,
                            help='If 1, train only MSF modules; freeze all other parameters')
        parser.add_argument('--train_msf_mugru_only', default=0, type=int,
                            help='If 1, train only MSF modules and MU-GRU motion_gate_conv; freeze all other parameters')
        parser.add_argument('--train_improvements_only', default=0, type=int,
                            help='If 1, train only MSF + MU-GRU + BGMA modules (all improvements); freeze all other parameters')
