...
parser.add_argument('--max_mid_term_frames', help='T_max in paper, decrease to save memory', type=int, default=17)
parser.add_argument('--min_mid_term_frames', help='T_min in paper, decrease to save memory', type=int, default=8)
parser.add_argument('--max_long_term_elements', help='LT_max in paper, increase if objects disappear for a long time', 
                                                type=int, default=10000)
parser.add_argument('--num_prototypes', help='P in paper', type=int, default=128)

parser.add_argument('--top_k', type=int, default=30)
parser.add_argument('--mem_every', help='r in paper. Increase to improve running speed.', type=int, default=3)
...
# Normalize model path to allow "no model" runs (random initialization)
if args.model is None or str(args.model).lower() in ['none', 'null', ''] or not os.path.isfile(str(args.model)):
    print('No valid model path provided. Proceeding with randomly initialized weights.')
    args.model = None
...
# Load our checkpoint (or run with random init if args.model is None)
network = XMem(config, args.model).cuda().eval()
if args.model is not None:
    model_weights = torch.load(args.model)
    network.load_weights(model_weights, init_as_zero_if_needed=True)
else:
    print('No model loaded (random initialization).')
