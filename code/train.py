...
for si, stage in enumerate(stages_to_perform):
    ...
    # Previous seeds: 14159265 (good), 20251021 (current)
    ...
    def renew_vos_loader(max_skip, finetune=False):
        ...
        # Detect if YouTubeVOS exists; if not, train on DAVIS only
        yv_img_dir = path.join(yv_root, 'JPEGImages')
        yv_ann_dir = path.join(yv_root, 'Annotations')
        yv_available = path.isdir(yv_img_dir) and path.isdir(yv_ann_dir)
        ...
        if yv_available:
            ...
            print('YouTubeVOS not found. Training on DAVIS only.')
    ...
    if stage == '0':
        ...
        davis_root = path.expanduser(config['davis_root'])
    ...
    # Previous implementation: np.random.seed(np.random.randint(2**30-1) + local_rank*100)
    np.random.seed(12345 + local_rank*100)
    try:
